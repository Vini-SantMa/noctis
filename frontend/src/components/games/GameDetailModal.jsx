...
export default function GameDetailModal({
  ...
  const [hours, setHours] = useState(libEntry?.hoursPlayed || libEntry?.horas_jogadas || '');
  ...
  const isRiotGame = game?.id?.toString().startsWith('riot-') || game?.developer?.toLowerCase().includes('riot');
  const dynamicTabs = [...BASE_TABS];
  if (isRiotGame) {
    dynamicTabs.push({ id: 'riot_stats', label: 'Estatísticas Riot' });
  }

  
  // Parsing pra pegar conquistas
  
  let parsedAchievements = {};
  try {
    parsedAchievements = typeof libEntry?.conquistas === 'string' 
      ? JSON.parse(libEntry.conquistas) 
      : (libEntry?.conquistas || {});
  } catch(e) {
    console.warn("Falha ao ler conquistas do banco:", e);
  }
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      let rawDesc = finalGame.description || finalGame.description_raw || '';

      try {
        // tentativa de buscar logo pelo id
        let details = await getGameDetails(initialGame.id).catch(() => null);

        const isGenericDesc = rawDesc.toLowerCase().includes('sincroniza') || rawDesc.length < 60;
        
        if (!details && (!rawDesc || isGenericDesc)) {
          
          // api rawg
          const apiKey = "f72b1bbe34aa416c8a16705624de5b7f"; 
          
          if (apiKey) {
            const query = encodeURIComponent(initialGame.title || initialGame.name);
            const searchRes = await fetch(`https://api.rawg.io/api/games?search=${query}&key=${apiKey}`);
            const searchData = await searchRes.json();
            
            if (searchData.results && searchData.results.length > 0) {
              const rawgGameId = searchData.results[0].id;
              details = await getGameDetails(rawgGameId); // Puxa os dados reais!
            }
          } else {
             console.warn("A chave da API do RAWG não foi inserida no Modal!");
          }
        }

        // 2. Mescla os dados encontrados ==> importante pra quando nao consegue pegar da steam
        if (details) {
          const idOriginal = finalGame.id;
          finalGame = { ...finalGame, ...details, id: idOriginal };
          rawDesc = details.description || details.synopsis || rawDesc;
          
        }
      } catch (err) {
        console.error("Erro na agregação de dados:", err);
      }

      setGame(finalGame);

      // Adaptação e atualizar a interface
      if (rawDesc && !rawDesc.toLowerCase().includes('sincroniza')) {
        const translated = await translateToPtBr(rawDesc);
        setDescription(translated || rawDesc);
      } else {
        setDescription('Nenhuma sinopse detalhada foi encontrada no momento para este título.');
      }
  ...
  const status = libEntry?.status || libEntry?.status_principal;
      ...
      <div className="fixed inset-0 md:pl-64 z-[50] flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="relative flex-shrink-0" style={{ height: '160px' }}>
            <img src={game?.cover || game?.background_image} alt={game?.title} className="w-full h-full object-cover object-top" />
            ...
            <div className="absolute bottom-3 left-4 right-12">
              <h2 className="text-white font-black text-xl leading-tight drop-shadow-lg">{game?.title || game?.name}</h2>
          ...
          <div className="flex items-center gap-2 px-4 py-3 flex-shrink-0 border-b border-white/[0.06]">
            ...
            {loadingDetail && (
              <span className="ml-auto flex items-center gap-1.5 text-white/30 text-xs">
                <Loader2 className="w-3 h-3 animate-spin" /> buscando agregados...
              </span>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4" style={{ WebkitOverflowScrolling: 'touch' }}>
            ...
            {tab === 'info' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="flex flex-wrap gap-2">
                  {game?.genres?.map(g => (
                    <span key={typeof g === 'string' ? g : g.name} className="px-3 py-1 rounded-full text-[11px] font-bold bg-purple-500/20 text-purple-300 border border-purple-500/30">
                      {capitalize(typeof g === 'string' ? g : g.name)}
                    </span>
                  ))}
                </div>
                ...
                <div>
                  {loadingDetail ? (
                    <div className="flex items-center gap-2 text-white/30 text-sm">
                      ...
                      Agregando dados de fontes externas...
                    </div>
                  ) : (
                    <p className="text-white/70 text-sm leading-relaxed max-h-40 overflow-y-auto pr-2" dangerouslySetInnerHTML={{ __html: description }} />
                  )}
                </div>
                ...
                <div className="p-4 rounded-2xl border border-white/[0.07] bg-white/[0.03]">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-white/[0.06] border border-white/10 rounded-xl px-4 py-2.5 text-white text-base focus:outline-none focus:border-purple-500/40"
                    />
            ...
            {tab === 'rating' && (
              <div>
                {/* Regras visuais no front para aplicacao do STATE */}
                {status === 'Quero Jogar' ? (
                  <div className="text-center py-12 animate-in fade-in duration-300">
                    ...
                    <h3 className="text-white font-bold mb-2">Avaliação Bloqueada</h3>
                    <p className="text-white/50 text-sm max-w-sm mx-auto mb-6">
                      De acordo com as regras do sistema, não é possível dar uma nota para um jogo que ainda está na lista de "Quero Jogar". 
                    </p>
                    <div className="inline-block px-4 py-2 border border-yellow-500/20 bg-yellow-500/10 rounded-xl text-yellow-400/80 text-xs uppercase tracking-widest font-semibold">
                      Altere o status para "Jogando" ou "Jogado"
                    </div>
                  </div>

                /* Se já jogou, pode avaliar*/
                ) : libEntry?.criteria ? (
                ...
                /* Se ainda nao avaliou, mas pode avaliar */
                ) : (
                  <div className="text-center py-12 animate-in fade-in duration-300">
            ...
            {tab === 'achievements' && (
              <AchievementsPanel
                game={game}
                libEntry={{ ...libEntry, conquistas: parsedAchievements }}
            ...
            {/* Aba para RIOT */}
            {tab === 'riot_stats' && (
              <div className="p-6 text-center border border-[#d32936]/30 bg-[#d32936]/10 rounded-2xl animate-in fade-in zoom-in duration-300">
                <div className="w-16 h-16 bg-[#d32936] text-white font-black text-2xl rounded-full flex items-center justify-center mx-auto mb-4 shadow-[0_0_20px_rgba(211,41,54,0.4)]">R</div>
                <h3 className="text-white font-bold mb-2 text-lg">Integração Riot Games</h3>
                <p className="text-white/60 text-sm mb-6">As estatísticas de KDA, Elo, Campeões Mais Jogados e Histórico de Partidas aparecerão aqui assim que o State e a RiotFactory estiverem conectados!</p>
                <div className="inline-block px-4 py-2 border border-white/10 rounded-xl text-white/40 text-xs uppercase tracking-widest">
                  Em Desenvolvimento
                </div>
              </div>
            )}
